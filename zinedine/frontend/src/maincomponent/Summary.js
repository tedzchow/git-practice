 ...
 const Summary=()=>{
   ...
   const desplay2=()=>{
    ...
    exps.forEach(exp => {
        ...
        if(Date.parse(new Date(exp.date))>=Date.parse(new Date(from))&&Date.parse(new Date(exp.date))<=Date.parse(new Date(to))){
            setnewexp(e=>[...e,exp]);
        }
    });
    }
        ...
        <div className="container">
            <div className="row">
                <div className="col mt-5">
                    ...
                    <button className="btn btn-success m-3" onClick={()=>desplay1()}>Desplay</button>
                    </div>
                ...
                <div className="col mt-5">
                    <div>
                    <Inputgroup type='date' label="From:" value={from} onChange={(e)=>setfrom(e.target.value)}></Inputgroup>~
                    <Inputgroup type='date' label="To:" value={to} onChange={(e)=>setto(e.target.value)}></Inputgroup>
                    <button className="btn btn-success m-3" onClick={()=>desplay2()}>Desplay</button>
                    </div>
